        ...
        var U = function() {
            ...
            this.activate = function(selector, classname, callback) {
                ...
                return className.add = function(selectorOrDom, className) {
                    for (var nodes = _.select(selectorOrDom), i = 0; i < nodes.length; i++) nodes[i].classList ? DOMTokenList.prototype.add.apply(nodes[i].classList, _.spliteAndTrim(className)) : nodes[i].className.indexOf(className) === -1 && (nodes[i].className = nodes[i].className + " " + className);
                ...
                var vendorPrefixProperties = [ "transition", "transform" ], vendors = [ "webkit", "Moz", "o", "ms" ], fn = function(selectorOrDom, style) {
                    for (var node, nodes = this.select(selectorOrDom), i = 0; node = nodes[i]; i++) for (var k in style) vendorPrefixProperties.indexOf(_.camelCase(k)) == -1 ? node.style[_.camelCase(k)] = style[k] : _.each(vendors, function(vendor) {
            ...
            }(this), this.localStorage = function(_, undefined) {
                var fn = function() {};
